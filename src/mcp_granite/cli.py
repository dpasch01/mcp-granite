"""CLI entry point for MCP-GRANITE."""
...
def main(verbose: bool) -> None:
    """MCP-GRANITE: Benchmark for tool granularity effects on LLM agents."""
...
def run(
    ...
    from mcp_granite.models.registry import preflight_check_models
    ...
    from mcp_granite.orchestrator.runner import run_experiment_sync
...
def evaluate(results_dir: Path) -> None:
    ...
    from mcp_granite.orchestrator.results import results_to_dataframe
...
def list_scenarios(domain: str | None) -> None:
    ...
    from mcp_granite.scenarios.loader import load_scenarios
...
def list_models() -> None:
    ...
    from mcp_granite.models.registry import MODELS
...
def list_runs(results_dir: Path) -> None:
    ...
    from mcp_granite.orchestrator.results import scan_all_runs
...
@click.option(
    ...
    type=str,
    default=None,
    help="Prometheus URL (overrides MCP_GRANITE_PROMETHEUS_URL env var).",
)
...
def fetch_metrics(
    ...
    from mcp_granite.monitoring.prometheus import PrometheusClient
    ...
    if not prometheus_url:
        ...
        prometheus_url = os.environ.get("MCP_GRANITE_PROMETHEUS_URL")
    if not prometheus_url:
        raise click.ClickException(
            "No Prometheus URL provided. Use --prometheus-url, config, or MCP_GRANITE_PROMETHEUS_URL env var."
        )
...
def serve_mcp(domain: str, granularity: str, fault_rate: float, seed: int) -> None:
    ...
    module = f"mcp_granite.mcp_servers.{domain}.{granularity}_server"
    try:
